const markupCreators = {
  playCard: (item, page) => {
    ...
    `<div class="play-card" data-id="${item.id}"> 
       ...
       ${page === "books" ? "<button class='base-button base-button--secondary cards-book-button' data-id='${item.id}'>Book now</button>" : ""} 
     </div>`:
    `<div class="play-card" data-id="${item.id}"> 
       <div class="play-card-image-wrapper"> 
         <img class="play-card-image"  src="${item.image}" >
...
bookPlayCardAdmin: (item) => {
  return`
  <div class="play-card"">    
      <h4 class='play-card-description'><b>${item.bookId}</b></h4> 
      <p class='play-card-description'>${item.bookingDate}</p>
      <p class='play-card-description'> ${item.userId}</p>    
  </div> 
`;
}
};
