...
const $adminPlayBooksContainer = document.querySelector('.admin__play-books-container');
...
async function handleGetPlays() {
  ...
  console.log(data);
  renderData(data,dataBook);
  console.log("dataBook", dataBook);

}

async function handleGetBooksAdmin() {  
  const dataBook = await Booking.getAdminBooks();     renderData(data,dataBook);
  console.log("dataBook", dataBook);
  
}

function renderData(data,dataBook) {
  setHtmlFromArray($adminPlayListContainer, data, markupCreators.playCard);
  setHtmlFromArray($adminPlayBooksContainer, dataBook, markupCreators.bookPlayCardAdmin);
  renderedData = data;
  cardProcess();
