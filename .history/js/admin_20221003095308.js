...
const $formElements = document.forms["admin-form"].getElementsByTagName("input");
const $buttons = document.forms["admin-form"].getElementsByTagName("button");
const $play = JSON.parse(localStorage.getItem("plays"))[editId];
...
for (let i = 0; i < cardsEdit.length; i++) {
  ...
  cardsEdit[i].addEventListener('click', async (e) => {
    ...
    console.log(play);
    $formElements[0].value = $play.name;
    $formElements[1].value = play.date;
    $formElements[2].value = play.price;
    $buttons[0].style.display = "none";
    $buttons[1].style.display = "block";
    $buttons[1].addEventListener('click', () => Play.editPlay(editId, play.image));
  });
}
